...
const PatientLogs: React.FC = () => {
    ...
    async (page: number = 0) => {
      ...
      try {
        const response = await fetchAllLogs(
          ...
          caregiverName || null,
          selectedLogType === "all" ? null : selectedLogType,
          patientName || null,
          startDate || null,
          endDate || null,
          "desc",
        ...
        setJumpPage(response.pagination.pageNo + 1);
      } catch (error) {
  ...
  const goToPage = (page: number) => {
    if (page >= 0 && page < logsData.pagination.totalPages) {
      handleLogs(page);
    }
  };
  ...
  const handleJump = () => {
    const page = Math.max(
      1,
      Math.min(jumpPage, logsData.pagination.totalPages)
    );
    goToPage(page - 1);
  };
                ...
                {logsData.pagination.totalPages > 0 && (
                  <div className="flex items-center justify-end gap-2 mt-4">
                    ...
                    <input
                      ...
                      min={1}
                      max={logsData.pagination.totalPages}
                      value={jumpPage}
                      onChange={(e) => setJumpPage(Number(e.target.value))}
                    />
                    <span className="text-sm text-muted-foreground">
                      of {logsData.pagination.totalPages}
                    </span>
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      disabled={logsData.pagination.pageNo <= 0}
                      onClick={() => goToPage(logsData.pagination.pageNo - 1)}
                    >
                      Prev
                    </Button>
                    <Button
