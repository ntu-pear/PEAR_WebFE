...
export default function ManageActivities() {
  ...
  const [deletedFilter, setDeletedFilter] =
    useState<"hidden" | "all" | "only">("hidden");
  ...
  useEffect(() => {
    setPage(1);
  }, [search, deletedFilter, data, searchFields]);

  const rows = useMemo(() => {
    let mapped = toRows(data ?? []);

    if (deletedFilter === "hidden") {
      mapped = mapped.filter(r => !r.is_deleted);
          ...
          <div className="flex items-center gap-3 w-full mb-4">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className="h-8"
                onClick={() =>
                  setSearchFields(p => ({ ...p, title: !p.title }))
                }
              >
                Title
              </Button>
