...
const privacySettingsHeaderCols = ["Social History", "Privacy Setting"];

const privacyLevelHeaderCols = ["Role", "Privacy Level"];
...
const ManageSocialHistory: React.FC = () => {
  ...
  const privacyLevelRows = useMemo(
    () =>
      roles.data
        ?.filter(
          (role) => role.roleName !== "ADMIN" && role.roleName !== "GUARDIAN"
        )
        .map((role) => ({
          name: role.roleName,
          label: role.roleName,
          id: role.id,
          privacyLevel: role.privacyLevelSensitive,
        })),
    [roles.data]
  );
  const rolesPrivacyLevel = useMemo(
    () =>
      roles.data?.reduce(
        (acc, role) => {
          acc[role.roleName] = role.privacyLevelSensitive.toString() as
            | "0"
            | "1"
            | "2"
            | "3";
  ...
  const privacySettingsRows = useMemo(
    () =>
      socialHistorySensitiveMapping.data?.map((item) => ({
  ...
  const socialHistoryPrivacySettings = useMemo(
    () =>
      socialHistorySensitiveMapping.data?.reduce(
        (acc, item) => {
  ...
  const privacySettingsForm = useForm<PrivacySettingsForm>({
    values: socialHistoryPrivacySettings as PrivacySettingsForm,
  });
  const privacyLevelForm = useForm<PrivacyLevelForm>({
    values: rolesPrivacyLevel as PrivacyLevelForm,
  });
  const updateRolePrivacyLevel = useUpdateRolePrivacyLevel();
  const updateSocialHistorySensitiveMapping =
    useUpdateSocialHistorySensitiveMapping();
  const [privacySettingsTooltipVisible, setPrivacySettingsTooltipVisible] =
    useState(false);
  const [privacyLevelTooltipVisible, setPrivacyLevelTooltipVisible] =
    useState(false);

  const onSubmitPrivacySettings: SubmitHandler<PrivacySettingsForm> = (
    data
  ) => {
    const socialHistoryMappings = Object.entries(data).map(
      ([socialHistoryItem, isSensitive]) => ({
        socialHistoryItem,
            ...
            <CardContent className="flex flex-col align-middle">
              <Card className="m-3">
                <CardHeader className="bg-sky-400 py-3 text-white font-semibold flex flex-row gap-2">
                  ...
                  <div className="relative flex items-center">
                    <div
                      ...
                      onMouseEnter={() =>
                        setPrivacySettingsTooltipVisible(true)
                      }
                      onMouseLeave={() =>
                        setPrivacySettingsTooltipVisible(false)
                      }
                    >
                      <InfoIcon />
                    </div>
                    {privacySettingsTooltipVisible && (
                      <div className="absolute bottom-full mb-2 w-96 bg-gray-700 text-white text-xs rounded py-1 px-2">
                        <h1>
                          Configure sensitivity of each social history
                          information.
                        </h1>
                        <h1>
                          Non-sensitive social history information can be view
                          by Caregivers, Doctors and Supervisors regardless of
                          privacy level.
                        </h1>
                      </div>
                    )}
                ...
                <CardContent className="py-4 flex flex-col">
                  <form
                    onSubmit={privacySettingsForm.handleSubmit(
                      onSubmitPrivacySettings
                    )}
                    ...
                    <Table className="border mb-2">
                      <TableHeader>
                        <TableRow>
                          {privacySettingsHeaderCols.map((col) => (
                            <TableHead className="border" key={col}>
                              {col}
                            </TableHead>
                          ))}
                        </TableRow>
                      ...
                      <TableBody>
                        {privacySettingsRows?.map((row) => (
                          <TableRow
                            key={row.name}
                            className="odd:bg-slate-100 odd:dark:bg-slate-700"
                          >
              ...
              <Card className="m-3">
                  ...
                  <div className="relative flex items-center">
                    ...
                    {privacyLevelTooltipVisible && (
                      <div className="absolute bottom-full mb-2 w-40 bg-gray-700 text-white text-xs rounded py-1 px-2">
                        <h1>Select the privacy level of each role.</h1>
                      </div>
                    )}
                  </div>
                </CardHeader>
                <CardContent className="py-4 flex flex-col">
                  <form
                    onSubmit={privacyLevelForm.handleSubmit(
                      onSubmitPrivacyLevel
                    )}
                    className="flex flex-col items-center"
                  >
                    <Table className="border mb-2">
