...
export const CENTER_OPEN_HOUR = 6;
export const CENTER_CLOSE_HOUR = 22;

// Time slot definitions for Week/Day view itself
// Updated to show time ranges (start - end time)
export const TIME_SLOTS = Array.from({ length: CENTER_CLOSE_HOUR - CENTER_OPEN_HOUR }, (_, i) => {
  const startHour = CENTER_OPEN_HOUR + i;
...
export const API_TIME_SLOTS = Array.from({ length: CENTER_CLOSE_HOUR - CENTER_OPEN_HOUR + 1 }, (_, i) => CENTER_OPEN_HOUR + i);
...
export const ACTIVITY_STYLES = {
  ...
  baseActivity: 'rounded-md px-1 text-xs cursor-pointer shadow-sm',
